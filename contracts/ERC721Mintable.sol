...
abstract contract Ownable {
    ...
    modifier onlyOwner() {
			require(
				msg.sender == _owner,
				"Only owner allowd to transfer ownership"
			);
			_;
    }
    ...
    function transferOwnership(address newOwner) public onlyOwner {
			require(
					newOwner != address(0),
					"New owner needs to be a valid address"
			);
			require(
					newOwner != _owner,
					"New owner needs to be a different address"
			);
			_owner = newOwner;
...
abstract contract Pausable is Ownable {
	...
	function setPausedStatus(bool paused) onlyOwner public {
		bool differs = _paused != paused;
		_paused = paused;
		if (differs && _paused){
			if (_paused) {
...
abstract contract ERC165 is Pausable {
...
abstract contract ERC721 is Pausable, ERC165 {
	...
	function approve(address to, uint256 tokenId) whenNotPaused public {
		...
		require(to != owner, "to address can't be the current owner");
		require(to != address(0), "to address needs to be a valid address");
		require(msg.sender == _owner || isApprovedForAll(owner,msg.sender), "msg.sender is neither the contract owner nor appoved for all");
	...
	function _mint(address to, uint256 tokenId) public virtual returns(bool) {
	...
	function _transferFrom(
		...
		require(to != address(0), "to must be a valid address");
...
abstract contract ERC721Enumerable is ERC165, ERC721 {
	...
	/**
		...
		*/
	function _mint(address to, uint256 tokenId) public virtual override returns(bool) {
			bool result = super._mint(to, tokenId);
			_addTokenToOwnerEnumeration(to, tokenId);
...
contract ERC721Metadata is ERC721Enumerable {
	...
	function setTokenURI(uint256 tokenId, string memory uri) whenNotPaused internal 
	{
...
contract ERC721MintableComplete is ERC721Metadata {
	...
	function _mint(address to, uint256 tokenId, string memory tokenURI) onlyOwner whenNotPaused public virtual
	returns(bool)
	{
